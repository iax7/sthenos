...
const sizes = {
  default: 'px-4 py-2 shadow-md',
  icon: 'size-8 border border-transparent text-sm text-gray-500',
}
</script>
...
<template>
  <button :type="props.type" :disabled="props.disabled" :class="[
    base,
    sizes[props.variant === 'icon' ? 'icon' : 'default'],
    {
      ...
      'text-gray-500 hover:text-gray-700 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500': props.variant === 'icon',
    },
    props.class
  ]">
    <slot />
