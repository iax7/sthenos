...
<template>
  <div class="relative inline-flex items-center group">
    <svg :width="props.size" :height="props.size" :viewBox="`0 0 ${props.size} ${props.size}`" class="cursor-help">
      ...
      <text v-if="props.showText" :x="radius" :y="radius + 3" text-anchor="middle" font-size="9" :fill="meta.textColor"
        font-weight="900">
        {{ t(meta.shortKey) }}
      </text>
    </svg>
    <div
      class="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded bg-gray-800 px-2 py-1 font-medium text-gray-100 opacity-0 group-hover:opacity-100 transition">
      {{ t(meta.labelKey) }}
    </div>
  </div>
</template>
